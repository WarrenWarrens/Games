...
from pgzero.builtins import Actor, animate, keyboard, music, sounds
...
class InputState:
    ...
    Contains both level (currently held) and edge (just pressed) inputs.
    """
    left: bool          # Level: left arrow currently held
    right: bool         # Level: right arrow currently held
    jump_pressed: bool  # Edge: up arrow pressed this frame
    fire_pressed: bool  # Edge: space pressed this frame (to create orb)
    fire_held: bool     # Level: space currently held (to blow orb further)
    pause_pressed: bool # Edge: P pressed this frame (to toggle pause)


class InputManager:
    """
    Centralizes input reading and edge detection.
    Builds an InputState snapshot once per frame.
    """
    def __init__(self):
        # Track previous frame's button states for edge detection
    ...
    def get_input_state(self):
        """
        Read current keyboard state and build InputState with edge detection.
        Called once per frame before any game logic.
        """
        # Read current frame's raw input
        current_jump = keyboard.up
        current_fire = keyboard.space
        current_pause = keyboard.p
        
        # Edge detection: button pressed this frame = currently down AND was up last frame
        jump_pressed = current_jump and not self.prev_jump
        fire_pressed = current_fire and not self.prev_fire
        pause_pressed = current_pause and not self.prev_pause
...
class Player(GravityActor):
    ...
    def update(self, input_state):
        """
        Update player logic based on input snapshot.
        No direct keyboard access - all input comes from input_state parameter.
        """
        ...
        if self.hurt_timer > 100:
            ...
            # We're not hurt
            # Get input from InputState. dx represents the direction the player is facing
            dx = 0
            if input_state.left:
                dx = -1
            ...
            # Do we need to create a new orb? Space must have been pressed (edge), the minimum time between
        ...
        # Holding down fire button causes the current orb (if there is one) to be blown further
        if input_state.fire_held:
            ...
            # If we let go of fire, we relinquish control over the current orb - it can't be blown any further
            self.blowing_orb = None
...
class Game:
    ...
    def update(self, input_state):
        """Update game logic with input snapshot"""
        self.timer += 1

        # Update all objects - player receives input_state
...
# ============================================================================
# SCREEN OBJECTS (State Pattern Implementation)
# ============================================================================

class Screen:
    """Base class for all screen objects"""
    def update(self, input_state):
        """Update screen logic with input snapshot"""
...
class MenuScreen(Screen):
    ...
    def __init__(self, app):
        self.app = app
        self.game = Game()  # Game object without player for background animation
    
    def update(self, input_state):
        """Update menu screen - check for fire button to start game"""
        if input_state.fire_pressed:
            # Transition to play screen
            self.app.change_screen(PlayScreen(self.app))
        else:
            # Update background animation (pass empty input since no player exists)
            dummy_input = InputState(False, False, False, False, False, False)
            self.game.update(dummy_input)
...
class PlayScreen(Screen):
    """Active gameplay screen with pause functionality"""
    def __init__(self, app):
        self.app = app
        self.game = Game(Player())  # Game object with player
        self.paused = False
    
    def update(self, input_state):
        """Update gameplay - handle pause toggle and game over transitions"""
        # Check for pause toggle (only in PlayScreen)
        ...
        if not self.paused:
            if self.game.player.lives < 0:
                # Player has lost all lives - transition to game over
    ...
    def draw(self):
        """Draw game scene and pause overlay if paused"""
        self.game.draw()
        draw_status()
...
class GameOverScreen(Screen):
    ...
    def __init__(self, app, game):
        self.app = app
        self.game = game  # Keep reference to the game for displaying final score
    
    def update(self, input_state):
        """Check for fire button to return to menu"""
        if input_state.fire_pressed:
            # Return to menu
...
class App:
    """Main application that manages screen transitions and input"""
    def __init__(self):
    ...
    def update(self):
        """Build input snapshot and delegate update to current screen"""
        input_state = self.input_manager.get_input_state()
...
def update():
    """Global update function - thin delegate to App"""
    app.update()

def draw():
    """Global draw function - thin delegate to App"""
    app.draw()
